...
class SuspendAPIScreen extends Component {
  ...
  componentDidMount() {
    this.setState({
      condition: this.props.account.trade_suspended_by_user
        ? 'RECOVERING_API'
        : 'SUSPENDING_API',
    });
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (
      !this.props.account.trade_suspended_by_user &&
      nextProps.account.trade_suspended_by_user
    ) {
  ...
  static navigationOptions = (props) => {
    ...
    return {
      headerLeft:
        condition === 'SUSPENDING_API_SUCCESS' ? null : (
          <NavigationIcon
  ...
  renderContent = () => {
    ...
    const {configureAccount, fetching} = this.props;

    let content;
    if (condition === 'SUSPENDING_API') {
      content = (
        <View style={styles.container}>
          <Text style={styles.h1}>Suspending Your API</Text>
          <Text style={[styles.h3, {marginTop: size(20)}]}>
            You are suspending your API. This will stop any new orders to come
            in to the system.{'\n\n'}
            After suspending your API, you can recover it by clicking “RECOVER
            API” on Emergency tab.
          </Text>
          <Button
            style={styles.button}
      ...
      content = (
        <View style={styles.container}>
          <Text style={styles.label}>API Suspension Submitted</Text>
          <Text style={[styles.h3, {marginTop: 66}]}>
            Now, you have suspended your API.{'\n\n'}
            In order to recover your API, go to Emergency tab then click
            “RECOVER API”.
          </Text>
          <Button
            style={styles.button}
...
const mapDispatchToProps = (dispatch) => {
  return {
    configureAccount: (data) =>
      dispatch(AccountActions.configureAccountAttempt(data)),
  };
};
