...
class OrderItem extends Component {
  render() {
    ...
    const mainValue = `${order.qty} (${order.filled_qty} filled) | ${capitalize(
      order.type,
    )} ${capitalize(order.side)} ${capitalize(order.time_in_force)}`;
    const timeValue =
      capitalize(order.status) + ': ' + changeTimeFormat(order.submitted_at);
